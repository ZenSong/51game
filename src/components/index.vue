<template>
  <div class="index">  
    ...
    <div class="content_box">
        <van-tabs  @change="changeActive" :active=myActive  swipeable sticky class="my-box-shadow" color="#F99D00" title-active-color="#F99D00">
           ...
           <!-- 广告 对应分类里的广告 -->
            ...
            <ul class="home_specs" v-if="item.name=='Home'">
              <li v-for="(item2,index2)  in item.data " :key="index2"> 
                  ...
                  <div v-for="(pro,pro1) in adsData" :key="pro1" >
                      <div v-if="index2+1==pro.sort && pro.adarea=='top'" v-html="pro.instext" class="googleList"></div>
                  </div> 
                   <!-- 轮播图 -->  
                        ...
                        <van-swipe-item v-for="(item,index) in slides" :key="index">
                          <ul>
                            <li v-for="(pro) in item" :key="pro.id">
                              ...
                              <router-link :to="{path: `/type/${pro.name.trim().replace(/\s+/g,'-').replace(/[\+ | \? | \% | \& | \= | \# | \^ | \* | \! | \$|\/]/g,'')}/${pro.typeId}/${pro.id}`}" tag="a">
                                    <div>
                  ...
                  <ul>
                    <li v-for="(item3,index3 )  in item2 " :key="index3+item3">
                      <!-- <a :href="'http://fun.cnbj.xyz'+item3.url" v-if="item3.url.indexOf('https') == -1">
                        <dl >
                          <dt>  
                            <img v-lazy="item3.cover" alt=""> 
                            </dt>
                          <dd>{{item3.name}}</dd>
                          <dd><span> {{parseInt(item3.play_count/10000)}} W</span></dd>
                        </dl>
                      </a> 
                      <a :href="item3.url" v-if="item3.url.indexOf('https') != -1">
                        <dl >
                          <dt> 
                            <img v-lazy="item3.cover" alt=""> 
                            </dt>
                      ...
                      <router-link tag="a" :to="{path: `/type/${item3.name.trim().replace(/\s+/g,'-').replace(/[\+ | \? | \% | \& | \= | \# | \^ | \* | \! | \$|\/]/g,'')}/${item3.typeId}/${item3.id}`, }">
                        <dl >
                          <dt>  
                            <img v-lazy="item3.cover" alt=""> 
                            </dt>
                          <dd>{{item3.name}}</dd>
                          <dd><span> {{item3.play_count}}0K</span></dd>
                        </dl>
                      </router-link>
                    </li>
                  </ul>
                  ...
                  <div v-for="(item4,index4) in adsData" :key="index4+'buttom'" >
                    <div v-if="index2+1==item4.sort && item4.adarea=='buttom'" v-html="item4.instext" class="googleList">
                      {{item4.instext}}
                    </div>
                  </div>
              </li>
            ...
            <ul class="home_categories" v-if="item.name!='Home'">
                ...
                <li v-for=" (item2) in classifyGames" :key="item2.id">
                  ...
                  <router-link :to="{path: `/type/${item2.name.trim().replace(/\s+/g,'-').replace(/[\+ | \? | \% | \& | \= | \# | \^ | \* | \! | \$|\/]/g,'')}/${item2.typeId}/${item2.id}`}" tag="a" v-if="!item2.hasOwnProperty('url')">
                     <dl>
                      <dt> 
                        <img v-lazy="item2.cover" alt=""> 
                        </dt>
                      <dd class="massage">
                        <p>{{item2.name}}</p> 
                        <p><span> {{item2.play_count}}0K</span></p>
                      </dd>
                      <dd class="play">play</dd>
                    </dl> 
                  </router-link>
...
export default {
  ...
  destroyed(){
   ...
   methods: {
    ...
    // 旧数据
    get_homeData(){
      var url="thirdadver/thirdadver!getH5gamesInfo.action"
      this.$axios.get(url).then(res=>{
        ...
        //   }
        // ] 
        // this.homeSpecs.push(...res.data.categories)  
        // this.homeData=res.data 
        // this.slides=res.data.slides
        // this.homeSpecs[0].data[0].push(...res.data.hots)
        // this.homeSpecs[0].data[1].push(...res.data.bests)
        // this.homeSpecs[0].data[2].push(...res.data.latest)   
        // this.getNewData()
        // this.get_adsData()
      }).catch(err=>{
        console.log(err)
    ...
    //新增数据
     getNewData (bol) { 
      var url = 'thirdadver/thirdadver!getH5gamesInfo2.action'
      this.$axios.get(url)
      .then(res => {
        // // 旧数据要停 先这样调用 之前是注释掉这些代码的 
        // this.homeSpecs= [
        //   {
        //     id: 1,
        //     name: "Home",
        ...
        // this.homeSpecs[0] 是home第一个分类  this.homeSpecs[0].data[0]是hots下的数据
        this.homeSpecs[0].data[0].push(...res.data.hots)
        this.homeSpecs[0].data[1].push(...res.data.bests)
        this.homeSpecs[0].data[2].push(...res.data.latest) 
        ...
        this.homeSpecs.push(...res.data.categories)
        // this.classifyGames = res.data.categories  
        // this.get_homeData()  
        // 获取自己的数据
        // this.getUsData()
      })
    ...
    // 每点击一次分类就获取详情分类 
    getClassifyGames(){ 
      // var url="/thirdadver/thirdadver!getH5gamesCategory.action?id=" + this.queryID
      // this.$axios.get(url)
      // .then((res)=>{    
      //    if(res.data == "") { 
      //        var url1="/thirdadver/thirdadver!getH5gamesCategory2.action?id=" + this.queryID
      //       this.$axios.get(url1)
      //       .then((res1) => {      
      ...
      //    }
      //    else {
      //       this.classifyGames = res.data.games
      //       this.dynamicInsertion1()
      //    } 
    ...
    getUsClassifyGames(str) { 
        ...
        var url = "getGamesList?begin=" + this.pageNumber + "&end=15" + "&typeId=" + this.queryID
        this.$axios.get(url)
        .then(res=> {  
    ...
    dynamicInsertion1(){
     ...
     changeActive(index,title){  
      ...
      if(title != "Home") {
        ...
        // // 由于接了两家的数据 点击切换的时候不知道调用哪个接口 这边根据id的大小来调用  id大于等于30的是我们的数据
        // if(this.queryID >= 30) {
        //   // 判断是否滑动到底部
        //   window.addEventListener("scroll", this.MyScroll, false)
        //   this.getUsClassifyGames("first")
